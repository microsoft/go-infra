...
const (
	symbolKey = "" + SymbolNotStarted + " Waiting for first report, " +
...
// All recognized pipelines.
const (
	// Release pipelines.
	releaseBuildPipelineName            = "microsoft-go-infra-release-build (official)"
	releaseImagesPipelineName           = "microsoft-go-infra-release-go-images (official)"
	releaseInnerloopPipelineName        = "microsoft-go-infra-release-innerloop (official)"
	releaseStartPipelineName            = "microsoft-go-infra-release-start (official)"
	releaseUpdateAzureLinuxPipelineName = "microsoft-go-infra-update-azure-linux (official)"

	// microsoft/go pipelines.
	microsoftGoPipelineName          = "microsoft-go"
	microsoftGoInnerloopPipelineName = "microsoft-go-innerloop (unofficial)"

	// microsoft/go-images pipelines.
	microsoftGoImagesPipelineName = "microsoft-go-images"
)
...
// notificationPreamble generates the preamble for a notification comment based
// on the pipeline state and which pipeline is involved. Returns empty string if
// no notification to the release runner should be posted.
func (s *State) notificationPreamble() string {
	...
	case SymbolSucceeded:
		...
		case releaseInnerloopPipelineName,
			releaseStartPipelineName,
			microsoftGoPipelineName,
			microsoftGoInnerloopPipelineName,
			microsoftGoImagesPipelineName:
			return "" // The release runner doesn't need to take action.

		default:
			return "Build succeeded! Name is not recognized. Check [buildreport.go](https://github.com/microsoft/go-infra/blob/main/buildreport/buildreport.go).\n"
		}
	}
	return "" // Some other state, like in progress.
}
