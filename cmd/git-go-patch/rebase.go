...
With the -shell flag, this opens an interactive terminal session in the submodule directory instead
of running git rebase directly. When you exit the terminal, it automatically runs 'git go-patch extract'
...
func handleRebase(p subcmd.ParseFunc) error {
	shell := flag.Bool("shell", false, "Open an interactive shell in the submodule instead of running git rebase directly. When you exit, automatically runs 'git go-patch extract'.")
	...
	// Check if we're already in a git-go-patch shell session
...
func handleInteractiveShell(goDir, since string, skipExtract bool) error {
	...
	fmt.Printf("\nSuggested workflow:\n")
	fmt.Printf("  - Run 'git go-patch rebase' to start an interactive rebase\n")
