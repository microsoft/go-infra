	...
	for _, version := range ri.Versions {
		...
		downloadAssetMetadata := coordinator.NewStep(
			...
			func(ctx context.Context) error {
				...
				if err := sb.VerifyAssetVersion(ctx, vs.AssetJSONPath, version); err != nil {
					return err
				}
				vs.AssetJSONPath = filepath.Join(dir, "assets.json")
				return nil
			},
			officialBuild,
		)
		...
		downloadArtifacts := coordinator.NewStep(
			...
			func(ctx context.Context) error {
				...
				var err error
				vs.ArtifactsDir, err = sb.DownloadPipelineArtifactToDir(
					ctx,
					vs.OfficialBuildID,
					"Binaries Signed",
					secret,
		...
		githubPublish := coordinator.NewStep(
			...
			func(ctx context.Context) error {
				...
				err := sb.CreateGitHubRelease(ctx, ri.TargetRepo, vs.GitHubTag, vs.AssetJSONPath, vs.ArtifactsDir, secret)
				if err != nil {
					return err
				}
		...
		dockerfilePublish := coordinator.NewStep(
			...
			func(ctx context.Context) error {
				if vs.ImageUpdatePR == 0 {
					var err error
					vs.ImageUpdatePR, err = sb.CreateDockerImagesPR(ctx, ri.TargetRepo, vs.AssetJSONPath, "", secret)
					if err != nil {
						return err
					}
				}
