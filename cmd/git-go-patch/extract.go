...
Each command starts with "` + patch.CommandPrefix + `", then:

- "` + patch.PatchNumberCommand + `<x>"
...
- "` + patch.AutoVendorPrefix + ` <dir1> [<dir2> ...]"
  Mark this patch as an auto-vendor patch. During extract, diff hunks for vendor directories
  (e.g. <dir>/vendor/) are stripped from the patch file. During apply, "go mod vendor" is run
  in each listed module directory after applying the patch, and the commit is amended.

  Example: "` + patch.AutoVendorPrefix + ` src src/cmd" strips src/vendor/ and src/cmd/vendor/ from the
  extracted patch and regenerates them during apply.
...
func handleExtract(p subcmd.ParseFunc) error {
	...
	if err := patch.WalkPatches(tmpRawDir, func(path string) error {
		...
		cmds, err := patch.ReadPatchCommands(subjectReader)
		if err != nil {
			return err
		}
		var vendorModDirs []string
		for _, cmd := range cmds {
			...
			if after, found := stringutil.CutPrefix(cmd, patch.PatchNumberCommand); found {
			...
			} else if after, found := stringutil.CutPrefix(cmd, patch.AutoVendorPrefix); found {
				dirs := strings.Fields(after)
				if len(dirs) == 0 {
					return fmt.Errorf("auto vendor command requires module directories (e.g. %q) in %#q", "auto vendor src src/cmd", path)
				}
				vendorModDirs = dirs
			} else {
		...
		// If this patch has an auto-vendor command, filter out vendor directory
		// diffs. The vendor content will be regenerated by "go mod vendor"
		// during apply.
		if len(vendorModDirs) > 0 {
			vendorPaths := patch.VendorPathsFromModDirs(vendorModDirs)
			p.Content = patch.FilterVendorContent(p.Content, vendorPaths)
		}
