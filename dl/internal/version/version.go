...
func Run(version, assetJSONSHA256Sum string) {
	log.SetFlags(0)
	...
	if len(os.Args) == 2 && os.Args[1] == "download" {
		if err := install(root, version, assetJSONSHA256Sum); err != nil {
...
// directory as needed.
func install(targetDir, version, assetJSONSHA256Sum string) error {
...
func verifySHA256(file, wantHex string) error {
	...
	if gotHex := fmt.Sprintf("%x", hash.Sum(nil)); gotHex != wantHex {
		return fmt.Errorf("%s corrupt? does not have expected SHA-256 of %q, got %q", file, wantHex, gotHex)
	}
	return nil
}
