...
// equivalent to an existing patch.
type MatchCheckRepo struct {
	gitDir                     string
	existingPatchPathsByHeader map[Header]string

	// LastApplyExistingMatch is the match for the last patch file passed to the Apply method, or ""
	// if no match was found. The purpose is to provide "git go-patch extract" functionality that
	// ignores superficial differences like hunk line numbers and index hashes that would only
	// distract code reviewers.
	LastApplyExistingMatch string
}
...
// Apply applies the patch at path to the MatchCheckRepo, updating the LastApplyExistingMatch field.
func (m *MatchCheckRepo) Apply(path string, p *Patch) error {
	// Clear the last result no matter what return we hit.
	m.LastApplyExistingMatch = ""
	...
	if ok && oldCommit != "" {
		...
		m.LastApplyExistingMatch = oldPatchPath
	} else {
