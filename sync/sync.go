...
func MakeBranchPRs(f *Flags, dir string, entry *ConfigEntry) ([]SyncResult, error) {
	...
	for i, b := range branches {
		...
		if entry.SubmoduleTarget == "" {
			...
			// Only stage patches regenerated by a successful three-way retry. Clean patches and
			// unresolved patch sets remain unchanged to keep the sync diff focused.
			if oldSubmoduleCommit != newCommit {
				updatedPatches, err := patch.TryThreeWayRebase(dir, entry.SubmoduleTarget, newCommit)
				if err != nil {
					return nil, err
				}
				if len(updatedPatches) > 0 {
					if err := run(newGitCmd(append([]string{"add", "--"}, updatedPatches...)...)); err != nil {
						return nil, err
					}
				}
			}
