...
// Package goimagesdemo implements the narrowly allowlisted real demo workflow for the unofficial
// go-images pipeline. It cannot target the official pipeline or publish under public/.
...
const (
	// DefinitionID is the microsoft-go-images (unofficial) pipeline.
	DefinitionID = 1492
	// SourceBranch is the only source branch accepted by the demo service.
	SourceBranch = "refs/heads/microsoft/main"

	correlationVariable     = "ReleaseUIUnofficialDemoSessionID"
	executionDigestVariable = "ReleaseUIUnofficialDemoExecutionDigest"
	versionsVariable        = "ReleaseUIUnofficialDemoVersions"
	sourceBuildVariable     = "ReleaseUIUnofficialDemoSourceBuildID"
)
...
// QueueClient can only queue the hardcoded unofficial demo definition and payload.
type QueueClient interface {
	QueueUnofficialDemo(context.Context, QueueRequest) (int, error)
}
...
// New validates and creates an unofficial demo service.
func New(reader PipelineReader, queue QueueClient, config Config, sleeper Sleeper) (*Service, error) {
	if reader == nil || queue == nil {
		return nil, errors.New("unofficial demo pipeline reader and queue client are required")
	}
	if config.SessionID == "" || config.ExecutionDigest == "" {
		return nil, errors.New("unofficial demo session ID and execution digest are required")
	}
	if !commitPattern.MatchString(config.SourceVersion) {
		return nil, fmt.Errorf("invalid unofficial demo source commit %q", config.SourceVersion)
	}
	if id, err := strconv.Atoi(config.SourceBuildID); err != nil || id <= 0 {
		return nil, fmt.Errorf("invalid unofficial demo source build ID %q", config.SourceBuildID)
	}
...
// TriggerBuildPipeline reconciles this session before queueing the hardcoded unofficial pipeline.
func (s *Service) TriggerBuildPipeline(
	...
	if pipelineID != DefinitionID {
		return "", fmt.Errorf("pipeline %d is not the allowlisted unofficial demo definition %d", pipelineID, DefinitionID)
	}
	if len(optionalParameters) != 0 {
		return "", errors.New("unofficial demo pipeline does not accept optional parameters")
	}
	if !maps.Equal(parameters, releasesteps.GoImagesUnofficialDemoPipelineParameters()) {
		return "", fmt.Errorf("unofficial demo parameters are not allowlisted: %#v", parameters)
	}
	...
	buildID, err := s.queue.QueueUnofficialDemo(ctx, QueueRequest{
	...
	if err != nil {
		return "", fmt.Errorf("queue unofficial go-images demo: %w", err)
	}
...
func (s *Service) findCorrelatedBuild(ctx context.Context) (*azdopipeline.Build, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reconcile unofficial demo pipeline run: %w", err)
	}
...
func (s *Service) validateCorrelatedBuild(build *azdopipeline.Build) error {
	if build == nil || build.ID <= 0 || build.DefinitionID != DefinitionID {
		return fmt.Errorf("correlated unofficial demo build has invalid identity: %#v", build)
	}
	if build.SourceBranch != SourceBranch || build.SourceVersion != s.config.SourceVersion {
		return fmt.Errorf(
			"correlated unofficial demo build %d has source %s@%s, expected %s@%s",
	...
	for name, want := range map[string]string{
		...
		if got := build.Parameters[name]; got != want {
			return fmt.Errorf("correlated unofficial demo build %d has %s %q, expected %q", build.ID, name, got, want)
		}
	}
	for name, want := range releasesteps.GoImagesUnofficialDemoPipelineParameters() {
		if got, ok := templateParameterString(build.TemplateParameters[name]); !ok || got != want {
			return fmt.Errorf("correlated unofficial demo build %d has parameter %s %q, expected %q", build.ID, name, got, want)
		}
	}
	return nil
}
...
// PollPipelineComplete waits for the correlated unofficial run using read-only GET requests.
func (s *Service) PollPipelineComplete(ctx context.Context, buildID string, _ *releasesteps.Secret) error {
	...
	if err != nil || id <= 0 {
		return fmt.Errorf("invalid unofficial demo build ID %q", buildID)
	}
	for {
		build, err := s.reader.Get(ctx, id)
		if err != nil {
			return fmt.Errorf("get unofficial demo build %d: %w", id, err)
		}
		...
		if err != nil {
			return fmt.Errorf("interpret unofficial demo build %d: %w", id, err)
		}
		switch state {
		case azdopipeline.RunStateSucceeded:
			return nil
		case azdopipeline.RunStateFailed, azdopipeline.RunStateCanceled:
			return fmt.Errorf("unofficial demo build %d finished with state %q and result %q: %s", id, state, build.Result, build.WebURL)
		case azdopipeline.RunStateWaiting, azdopipeline.RunStateRunning:
			if err := s.sleep(ctx, s.config.PollInterval); err != nil {
				return err
			}
		default:
			return fmt.Errorf("unofficial demo build %d has unsupported state %q", id, state)
		}
	}
}
