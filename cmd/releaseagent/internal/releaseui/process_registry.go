...
// ProcessWorkflow describes an in-UI workflow. Server hooks are optional; inputs, steps, routes,
// state streaming, and rendering are shared.
	...
	return newProcessRegistry(
		...
		ProcessDefinition{
			...
			Workflow: &ProcessWorkflow{
				...
				Preflight: (*Server).handleGoInfraPreflight,
				GetPlan:   (*Server).handleGetGoInfraPlan,
				Prepare:   (*Server).handleGoInfraPlan,
				Start:     (*Server).handleGoInfraStart,
			},
		},
		ProcessDefinition{
...
func validateProcessWorkflow(processID string, workflow *ProcessWorkflow) error {
	...
	if workflow.Prepare != nil && strings.TrimSpace(workflow.SubmitLabel) == "" {
